...
(countpiece) If you are feeling tired to count piece value on board, than this command got you. Simply paste this command into welcome screen and 
...
(cls) Most useful and interesting command just type into console and it will wipe out all the screen and cleans everything on console for you.
(passnplay) Most hard thing before many things. Currently under development but you can use it how you play on a chess board but no take back and castling(It is planned for future).
...
Capital letters represent white pieces and small letters represents black pieces. If in between you want the current postition FEN then simply type (fen) on your turn and it will return you
the FEN. Want to give up simply type (resign).Brain is not braining of both you and your friend simply type (abort) to quit the game without any desicion. Feeling you are not going to win,
simply type(draw) and pray if opponent must accept.But remeber you have must played over fifteen moves and if opponent declines then you are not going to offer any till next fifteen moves.
